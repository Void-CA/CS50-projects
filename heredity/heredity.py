...
def update(probabilities, one_gene, two_genes, have_trait, p):
    ...
    for person in probabilities:
        gen = 2 if person in two_genes else 1 if person in one_gene else 0
...
def normalize(probabilities):
    ...
    """
    for person in probabilities:
        total_gen = sum([probabilities[person]["gene"][i] for i in probabilities[person]["gene"]])
        total_trait = sum([probabilities[person]["trait"][i] for i in probabilities[person]["trait"]])

        for i in [0,1,2]:
            probabilities[person]["gene"][i] /= total_gen
